...
public class BillionairesDemo extends AbstractDemo {
    public TreeMap _treeMap;

    public String getName() {
        return "TreeMap Example (Billionaires)";
    }

    public int getAttributes() {
        return ATTRIBUTE_BETA | ATTRIBUTE_NEW;
    }

    public Component getDemoPanel() {
        TableModel tableModel = createBillionairesTableModel();
        ...
        final MutableColorMap colormap = SwingColorMapFactory.getInstance().createColorMap(tableModel, 4);
        colormap.setPalette(SwingPaletteFactory.getInstance().createColorGradient("PuBu"));
    ...
    public Component getOptionsPanel() {
        ...
        // Customize how the label color are adjusted depending on the background
        final Labeling customLabeling = new CustomLabeling();
        LabelingFactory.getInstance().add(customLabeling);
        ...
        labelingComboBox.setSelectedItem(customLabeling);
    ...
    public BillionairesDemo() {
    }

    static public void main(String[] s) {
        showAsFrame(new BillionairesDemo());
    }
    ...
    private TableModel createBillionairesTableModel() {
        try {
            ...
            while (true);
            return new BillionairesTableModel(data, columnNames, columnTypes);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
    ...
    private static class BillionairesTableModel extends DefaultTableModel {
        private Vector columnTypes;

        public BillionairesTableModel(Vector data, Vector columnNames, Vector columnTypes) {
    ...
    private static class CustomLabeling extends OverlayLabeling {
        @Override
        protected Color adjustLabelColor(TreeMapFieldSettings settings, Color foreground, Color background) {
            int brightness = (int) Math.sqrt(
                    background.getRed() * background.getRed() * .241 +
                            background.getGreen() * background.getGreen() * .691 +
                            background.getBlue() * background.getBlue() * .068);
            if (brightness > 130) {
                return Color.black;
            } else {
